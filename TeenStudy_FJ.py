...
def GetSchoolInfo():                # 学校信息
    URL = "https://m.fjcyl.com/admin/user/orgList"
    info_rsp = session.post(url=URL, headers=headers, verify=False)
...
def LastestNum():  # 获取最新一期的青年大学习的id
    URL = "https://m.fjcyl.com/admin/cylOrgMembers/groupByList?groupBy=1"
    info = URL
    info_rsp = session.post(url=info, headers=headers, verify=False)
    info_json = info_rsp.json()
...
def LastestURL():  # 获取最新的链接，id是日期
    num = LastestNum()
    URL = "https://m.fjcyl.com/admin/cylOrgMembers/selectList?orderBy=1&quarterNo={}".format(num)
    info = URL
    info_rsp = session.post(url=info, headers=headers, verify=False)
    info_json = info_rsp.json()
    Ls = info_json["rs"]
...
def GetInfo(i):  # 获取没做名单
    num = LastestURL()
    URL = "https://m.fjcyl.com/admin/cylOrgMembers/selectCurrentStudy?studyId={}&" \
...
def Parma():  # 我们班有3页，所以就直接这样了
    ...
    Temp2 = GetInfo(2)
    Temp3 = GetInfo(3)
    TempT = Temp1 + Temp2 + Temp3  # 这里是将没有做的同学记录下来
    Temp = []
    for k in TempT:
        if k not in Temp:
            Temp.append(k)
    ...
    for i in range(len(Email)):
        if Email[i][1] in Temp:
            DidDoneEmail.append(Email[i][0])
    return DidDoneEmail
